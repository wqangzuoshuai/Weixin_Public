...
global inp1, inp2, inp3, inp4, inp5, inp6, inp7, inp8, inp9, inp11, inp13, inp14
...
def win():
    ...
    check2 = Checkbutton(win, text="每日一句", font=('微软雅黑', 8, 'bold'), variable=CheckVar2, onvalue=1, offvalue=0)
    ...
    # 定义执行函数
    def study():
        global app_id_out, app_secret_out, template_id_out, user_out, province_out, city_out, birthday1_out, birthday2_out, love_date_out, Whether_Eng_out, key1_out, user2_out, key2_out, key3_out, key5_out
        # 没有选择任何项目的情况下
        if (
                CheckVar1.get() == 0 and CheckVar2.get() == 0 and CheckVar3.get() == 0 and CheckVar4.get() == 0 and CheckVar5.get() == 0):
            s = '您还没选择任语言'
        else:
            s1 = "彩虹屁" if CheckVar1.get() == 1 else ""
            s2 = "每日一句" if CheckVar2.get() == 1 else ""
...
def get_json_data(json_path):
    ...
    with open(json_path, 'rb') as f:
        ...
        # 修改内容
        params["Whether_Eng"] = "\u5426"
        # 修改内容
        params["caihongpi_API"] = key1_out
        params["lizhi_API"] = "否"
        params["tianqi_API"] = "否"
        params["health_API"] = key1_out
        params["lucky_API"] = "否"
        params["astro"] = "\u91d1\u725b\u5ea7"
