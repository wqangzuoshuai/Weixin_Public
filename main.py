...
def get_weather(province, city):
    ...
    # 今日日期
    data = weatherinfo["fctime"]
    return weather, temp, tempn, data
...
def health():
    if (health_API != "否"):
        ...
        data = res.read()
        data = json.loads(data)
        data1 = data["newslist"][0]["note"]
        data2 = data["newslist"][0]["content"]
        return data1, data2
    else:
        return ""
...
def get_today():
    conn = http.client.HTTPSConnection('api.tianapi.com')  # 接口域名
    params = urllib.parse.urlencode({'key': health_API, 'city': "\u5317\u4eac"})
    headers = {'Content-type': 'application/x-www-form-urlencoded'}
    conn.request('POST', '/worldtime/index', params, headers)
    res = conn.getresponse()
    data = res.read()
    data = json.loads(data)
    today = data["newslist"][0]["strtime"]
    week = data["newslist"][0]["week"]
    return today, week


# 推送信息
def send_message(to_user, access_token, city_name, weather, data, max_temperature, min_temperature, pipi, lizhi, pop,
                 tips, note_en, note_ch, english1, english2, lucky_):
    url = "https://api.weixin.qq.com/cgi-bin/message/template/send?access_token={}".format(access_token)
    # week_list = ["星期日", "星期一", "星期二", "星期三", "星期四", "星期五", "星期六"]
    year = localtime().tm_year
    # month = localtime().tm_mon
    # day = localtime().tm_mday
    # today = data[:4] + "-" + data[4:6] + "-" + data[6:8]
    today, week = get_today()
    ...
    # 获取星期几
    week = "星期" + week
    # week = week_list[(today.isoweekday() % 7)+1]
    # 获取在一起的日子的日期格式
    ...
    for key, value in birthdays.items():
        # 获取距离下次生日的时间
        birth_day = get_birthday(value, today_year, today)
        # 将生日数据插入data
...
if __name__ == "__main__":
    ...
    weather, max_temperature, min_temperature, data = get_weather(province, city)
    # 获取彩虹屁API
    ...
    # 公众号推送消息
    for user in users:
        send_message(user, accessToken, city, weather, data, max_temperature, min_temperature, pipi, lizhi, pop, tips,
                     note_en, note_ch, english1, english2, lucky_)
    import time
